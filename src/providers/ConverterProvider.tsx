...
import { BookmarkBar, BookmarkItem, RootObject } from '../types';



export const convertContext = createContext<any>('');
export default function ConverterProvider({children}: {children:ReactNode}) {
    const [jsonContent, setJsonContent] = useState<RootObject>();
    ...
    const convertJsonToHTML = () => {
      ...
      try {
        ...
        function processBookmarks(folder: BookmarkItem | BookmarkBar) {
          folder.children?.forEach((bookmark) => {
            if (bookmark.type === 'folder') {
              ...
              html += '    </DL><p>\n';
            } else if (bookmark.type === 'url' && 'url' in bookmark) {
